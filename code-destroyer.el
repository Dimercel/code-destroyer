...
(defvar *cdg-ball-init-dir* (cdg-normalize-vec (vector +cdg-gap+ 1.0))
  "Исходное направление движения мяча. Его необходимо
  восстанавливать, всякий раз, как мяч улетает за пределы игровой зоны")
  ...
  (let ((inhibit-read-only t))
    ...
    (setq *cdg-platform*
          (cdg-make-platform
           ...
           ?-))
    (setq *cdg-ball*
          (cdg-return-ball-to-platform (cdg-make-ball (cdg-make-point 0 0)
                                                      *cdg-ball-init-dir*
                                                      ?o)
                                       *cdg-platform*
                                       *cdg-game-zone*))
...
(defun cdg-ball-out-p ()
  "Вылетел ли мяч за пределы игровой зоны.
  Такое может произойти лишь в случае, когда
